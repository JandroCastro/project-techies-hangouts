...
async function getHangoutAttendance(req, res, next) {
  ...
  try {
    ...
    const sqlQuery = `SELECT a.id_users as guest_id, age, link_url, avatar_url, e.user_id as organizator_id, position, p.name as userName,  e.id as hangout_id, request_status FROM Attendance a INNER JOIN Events e ON a.event_id = e.id LEFT JOIN Thematics t ON e.thematic_id = t.id LEFT JOIN Cities c ON e.city_id = c.id LEFT JOIN Profiles p ON a.id_users = p.user_id WHERE event_id = ?`;
